...
# ***************************** #
# CODE FOR APPLYING METHODOLGOY 2
# GIVING PROBABILITY BASED WEIGHTS TO EACH RETURN BASED ON ITS KELLY VALUE
# THE PROBABILITY IS FOUND OUT AS KELLY_VAL[i] OVER SUM OF KELLY VECTORS CONDITIONED ON IT BEING GREATER THAN 0.5
condtional_sum = sum(kellys_vector[kellys_vector > 1/2])
cumulative_return = c(1)
for (i in 2:length(kellys_vector)) {
  kelly_val = kellys_vector[i-1]
  if (kelly_val > 0.5) {
    weight = kelly_val / condtional_sum
  } else {
    weight = 0
  }
